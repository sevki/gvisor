...
//
//go:nosplit
func (w *checkWalker) iterateRange(start, end uintptr) {
	if start%pteSize != 0 {
...
//
//go:nosplit
func checknext(start uintptr, size uintptr) uintptr {
	start &= ^(size - 1)
	start += size
	return start
}
...
//
//go:nosplit
func (w *checkWalker) iterateRangeCanonical(start, end uintptr) {
	for pgdIndex := uint16((start & pgdMask) >> pgdShift); start < end && pgdIndex < entriesPerPage; pgdIndex++ {
		...
		for pudIndex := uint16((start & pudMask) >> pudShift); start < end && pudIndex < entriesPerPage; pudIndex++ {
			...
			if !pudEntry.Valid() {
				...
				if w.visitor.requiresSplit() && (start&(pudSize-1) != 0 || end < checknext(start, pudSize)) {
			...
			for pmdIndex := uint16((start & pmdMask) >> pmdShift); start < end && pmdIndex < entriesPerPage; pmdIndex++ {
				...
				if !pmdEntry.Valid() {
					...
					if w.visitor.requiresSplit() && (start&(pmdSize-1) != 0 || end < checknext(start, pmdSize)) {
